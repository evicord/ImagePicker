package com.tyrion.plugin.picker.ui;
...
import com.tyrion.plugin.picker.MyApplication;
import com.tyrion.plugin.picker.AppManager;

import com.tyrion.plugin.picker.common.ExtraKey;
import com.tyrion.plugin.picker.common.LocalImageHelper;
import com.tyrion.plugin.picker.widget.AlbumViewPager;
import com.tyrion.plugin.picker.widget.MatrixImageView;
                    ...
                    public void run() {
                        if (folders != null) {
                            ...
                            if (checkedItems.size()+LocalImageHelper.getInstance().getCurrentSize() > 0) {
                                finish.setText("完成(" + (checkedItems.size()+LocalImageHelper.getInstance().getCurrentSize()) + "/" + ExtraKey.IMAGE_CHOSE_COUNT +")");
                                finish.setEnabled(true);
                                headerFinish.setText("完成(" + (checkedItems.size()+LocalImageHelper.getInstance().getCurrentSize()) + "/" + ExtraKey.IMAGE_CHOSE_COUNT + ")");
...
//                                headerFinish.setEnabled(false);
    ...
    public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
        if (!b) {
            ...
            if (!checkedItems.contains(compoundButton.getTag())) {
                if(checkedItems.size()+LocalImageHelper.getInstance().getCurrentSize()>=ExtraKey.IMAGE_CHOSE_COUNT){
                    Toast.makeText(this,"最多选择" + ExtraKey.IMAGE_CHOSE_COUNT + "张图片", Toast.LENGTH_SHORT).show();
    ...
    public class MyAdapter extends BaseAdapter {
        ...
        public MyAdapter(Context context, List<LocalImageHelper.LocalFile> paths) {
            ...
            options=new DisplayImageOptions.Builder()
                    ...
                    .setImageSize(new ImageSize(((MyApplication) context.getApplicationContext()).getQuarterWidth(), 0))
